...
4321, 12331, 12421, 13321]
    ...
    string = str(number)
    lst = list(string)
    min_len = sum_of_digits  # define the minimum members in the combination

    if len(string) <= min_len:
        min_len = len(string)

    for i in range(1, min_len + 1):
        for item in combinations(lst, i):
            temp_int_item = [int(x) for x in item]
            if sum(temp_int_item) == sum_of_digits:
                num_str = ''
                # convert to a string
                for m in temp_int_item:
                    num_str += str(m)
                result.append(num_str)
    set_r = set(result)
    final_result = [int(item) for item in set_r]
    final_result.sort()
    return final_result
...
# print(subnumbers_whose_digits_add_up_to(13, 2))
# print(subnumbers_whose_digits_add_up_to(222, 2))
# print(subnumbers_whose_digits_add_up_to(123, 6))
# print(subnumbers_whose_digits_add_up_to(222, 4))
# print(subnumbers_whose_digits_add_up_to(1234, 5))
# print(subnumbers_whose_digits_add_up_to(12341234, 4))
# print(subnumbers_whose_digits_add_up_to(121212, 5))
# print(subnumbers_whose_digits_add_up_to(123454321, 10))
