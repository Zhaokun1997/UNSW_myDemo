...
def find_word(filename, word):
    ...
    LEMON was not found
    >>> find_word('word_search_2.txt', 'PABW')
    PABW was found diagonally (top left to bottom right) at position (10, 1)
    '''
    with open(filename) as file:
        grid = None
        # Insert your code here
        lines = [list(''.join(line.split())) for line in file if not line.isspace()]
        grid = np.array(lines)
        ...
        location = find_word_from_all_diagonals(grid, word)
        if location:
            found = True
...
def find_word_vertically(grid, word):
    new_grid = grid.T
    location = find_word_horizontally(new_grid, word)
...
def find_word_diagonally(grid, word):
    location = None
    for y_dim in range(len(grid)):
        row = ''.join(grid.diagonal(y_dim).tolist())
        index = row.find(word)
        if index != -1:
...
def find_word_diagonally_transpose(grid, word):
    location = None
    for y_dim in range(len(grid)):
        row = ''.join(grid.diagonal(y_dim).tolist())
        index = row.find(word)
...
def find_word_from_all_diagonals(gird, word):
    location_1 = find_word_diagonally(gird, word)
    location_2 = find_word_diagonally_transpose(gird.T, word)
    return location_1 or location_2
