...
def check_out_square_and_fix_if_corrupted(square):
    ...
    set_1 = set([e for row in square for e in row])
    set_2 = set([i for i in range(1, n ** 2 + 1, 1)])
    set_3 = set_1 - set_2
    set_4 = set_2 - set_1
    if len(set_3) == 0:
        good_square = True
    else:  # len(set_3) != 0
        if set_3 == {0} and len(set_4) == 1:
    ...
    elif not good_square and not corrupted_square:
        print(f'It is neither a good nor a corrupted square.')
    elif not good_square and corrupted_square:
