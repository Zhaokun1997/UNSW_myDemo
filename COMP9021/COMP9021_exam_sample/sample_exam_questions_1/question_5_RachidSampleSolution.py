    ...
    with open('cpiai.csv', 'r') as csv_file:
        ...
        for item in reader:
            if row_count == 0:
                ...
                if item[0].split('-')[0] == str(year):
                    if float(item[2]) > max_inf:
                        max_inf = float(item[2])
                    inflation.append(float(item[2]))
    max_Months = ''
    for i in range(len(inflation)):
        if inflation[i] == max_inf:
            max_Months += months[i] + ', '
    print(f'In {year}, maximum inflation was: {max_inf}')
    print(f'It was achieved in the following months: {max_Months[:-2]}')
    # print('In ' + str(year) + ', maximum inflation was:', max_inf)
    # print('It was achieved in the following months: ' + max_Months[:-2])
